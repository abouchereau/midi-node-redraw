...
module.exports = class Presets {
    ...
    static touchBoardVolcaTest = new MidiRemap((cmd, channel, param1, param2) => {
        let out = [];
        ...
        const LEAD_NOTE_SCALE = [57,59,60,62,64,65,67,69,71,72,74,76];
        if (cmd==MidiMsg.NOTE_ON) {
            ...
            if (param1 >= 48 && param1 <= 59) {
                out.push([MidiMsg.CC * 16 + LEAD_CHANNEL, CC_FLD, 90]);
                out.push([MidiMsg.CC * 16 + LEAD_CHANNEL, CC_PITCH_1, 1+LEAD_NOTE_SCALE[param1-48]-24]);
                out.push([MidiMsg.CC * 16 + LEAD_CHANNEL, CC_PITCH_2, 1+LEAD_NOTE_SCALE[param1-48]]);
