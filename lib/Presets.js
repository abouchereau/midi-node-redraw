    ...
    static touchBoardVolcaDaftPunk = new MidiRemap((cmd, channel, param1, param2) => {
        ...
        if (cmd==MidiMsg.NOTE_ON) {
            ...
            if (param1 >= 60 && param1 <= 63) {
                ...
                out.push([MidiMsg.CC * 16 + LEAD_CHANNEL, CC_PITCH_1, 1+LEAD_NOTE_SCALE[param1-64]-12]);
        ...
        return out;
            ...
            (ms, channel, note)=> {
                const CC_FLD = 50;
                const CHANNEL = 1;
                return [[MidiMsg.CC * 16 + CHANNEL, CC_FLD, Math.round(Graph.osc(ms, 63, 1500, 64, 0.95))]];
            },
            25,
