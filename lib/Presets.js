...
module.exports = class Presets {
    ...
    static touchBoardVolcaJMJMelody = new MidiRemap((cmd, channel, param1, param2) => {
        ...
        const BASS_NOTE_SCALE = [[41, 34, 36, 38], [31, 33, 34, 38]];//fa, sib, do, ré / sol, la, sib, ré
        const LEAD_NOTE_SCALE = [60, 62, 64, 65, 67, 69, 70, 72];
        if (cmd==MidiMsg.NOTE_ON) {
            console.log("ON", param1);
            //bass
            if (param1 >= 48 && param1 <= 51) {
                out.push([MidiMsg.CC * 16 + BASS_CHANNEL,CC_PITCH_1AND2, 1+BASS_NOTE_SCALE[Presets.bassScale][param1-48]]);
                if (param1 == 38 && Presets.lastBassNote == 36) { //après sib do, on passe sur la 2e scale
                    Presets.bassScale = 1;
                }
                if (param1 == 33 && Presets.lastBassNote == 31) { //après sol la, on repasse sur la 1er scale
                    Presets.bassScale = 0;
                }
                Presets.lastBassNote = param1;
            }
            //melody
    ...
    static touchBoardVolcaTransition = new MidiRemap((cmd, channel, param1, param2) => {
        ...
        if (cmd==MidiMsg.NOTE_ON) {
            ...
            if (param1 == 52) {
                ...
                for(let i of [1,2,4]) {
                    out.push([MidiMsg.CC * 16 + i, CC_LEV_1, 1]);
                    out.push([MidiMsg.CC * 16 + i, CC_LEV_2, 1]);
                }
                //on lance
