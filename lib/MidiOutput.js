...
module.exports = class MidiOutput {
    ...
    constructor() {
        ...
        this.timerTempo = new NanoTimer();
        this.timer.setInterval(() => {
            if (!this.stackIsEmpty) {
                let midiCmd = this.midiCmdStack.shift();
                if (this.midiOutput != null) {
                    this.midiOutput.send(midiCmd);
                    //console.log("MIDI OUT", JSON.stringify([midiCmd[0] >> 4,midiCmd[0] & 0xf,midiCmd[1],midiCmd[2]]));
                }
    ...
    avoidCCRepetition(tab) {
        this.ccAvoidRepetition = tab;
    }
    ...
    send(cmd) {
        ...
        let param1 = cmd[1];
        let param2 = cmd[2];
        if (com == MidiMsg.CC && this.ccAvoidRepetition.includes(param1)) {
    ...
    setTempo (tempo) {
        if (this.midiOutput != null) {
            ...
            this.timerTempo.clearInterval();
            this.timerTempo.setInterval(() => {
                this.midiOutput.send([MidiMsg.TICK]);
            }, '', tickDurationMicroNano + 'n');
        }
    }
